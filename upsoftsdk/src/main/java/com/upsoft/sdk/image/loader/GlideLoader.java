...
import com.upsoft.sdk.image.config.AnimationMode;
import com.upsoft.sdk.image.config.GlobalConfig;
import com.upsoft.sdk.image.config.PriorityMode;
import com.upsoft.sdk.image.config.ScaleMode;
import com.upsoft.sdk.image.config.ShapeMode;
import com.upsoft.sdk.image.config.SingleConfig;
import com.upsoft.sdk.image.utils.DownLoadImageService;
import com.upsoft.sdk.image.utils.ImageUtil;
...
public class GlideLoader implements ILoader {
    ...
    private DrawableTypeRequest getDrawableTypeRequest(SingleConfig config, RequestManager requestManager) {
        ...
        if (!TextUtils.isEmpty(config.getUrl())) {
            ...
            GlideUrl cookie = new GlideUrl(url, new LazyHeaders.Builder().addHeader("Cookie", CookieSP.getCookie
                    (config.getContext())).build());
            request = requestManager.load(cookie);
//            request = requestManager.load(ImageUtil.appendUrl(config.getUrl()));
    ...
    private void setShapeModeAndBlur(SingleConfig config, DrawableTypeRequest request) {
        ...
        if (config.isNeedSwirl()) {
            transformation[count] = new SwirlFilterTransformation(config.getContext(), 0.5f, 1.0f, new PointF(0.5f,
                    0.5f)); //漩涡
            count++;
        }
        ...
        switch (config.getShapeMode()) {
            ...
            case ShapeMode.RECT_ROUND:
                transformation[count] = new RoundedCornersTransformation
                        (config.getContext(), config.getRectRoundRadius(), 0, RoundedCornersTransformation.CornerType
                                .ALL);
                count++;
                break;
            case ShapeMode.OVAL:
    ...
    private int statisticsCount(SingleConfig config) {
        ...
        if (config.getShapeMode() == ShapeMode.OVAL || config.getShapeMode() == ShapeMode.RECT_ROUND || config
                .getShapeMode() == ShapeMode.SQUARE) {
            count++;
        }
