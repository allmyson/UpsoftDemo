...
public class UpdateManager implements View.OnClickListener {
    ...
    public UpdateManager(Context mContext, boolean showToast) {
        ...
        this.packageName = AppUtil.getPackageName(mContext);
        this.versionCode = AppUtil.getVersionCode(mContext, packageName);
    ...
    public void checkUpdate() {
        try {
            ...
            L.e("软件更新", params.toString());
            BaseHttp.getInstance().postJson((Activity) mContext, FunctionApi.getHttpIp(mContext) + Constant.Url
                    .UPDATA, params.toString(), new HttpListener<String>() {
                @Override
                public void onSucceed(int what, Response<String> response) {
                    try {
                        L.e("apk版本信息===", response.get());
                        UpdateBean baseBean = new Gson().fromJson(StringUtil.valueOf(response.get()), UpdateBean.class);
                        if ("1".equals(baseBean.status)) {
                            if (null != baseBean) {
                                downLoadUrl = UrlUtil.getFileUrlByFileId(currentIp, baseBean.data.downloadUrl);
                                compareVersionCode(versionCode,
                                        StringUtil.StringToInt(baseBean.data.versionNumber),
                                        isForceUpdate,
                                        baseBean.data.versionName,
                                        baseBean.data.updateDescription);
                            } else {
                                if (showToast) {
                            ...
                            ToastUtil.show(mContext, StringUtil.valueOf(baseBean.msg), 1);
                        }
                    } catch (Exception e) {
                        if (showToast) {
                            ToastUtil.show(mContext, "当前已是最新版本", 1);
                ...
                public void onFailed(int what, Response<String> response) {
                    if (showToast) {
                        ToastUtil.show(mContext, "网络连接超时", 1);
                    }
                }
            });
        } catch (Exception e) {
...
//                ".apk&sign=f36c7db67dbaa3031a1268f5f5284c3e&t=5bc98626";
    ...
    private void installApk(String path) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Uri contentUri = FileProvider.getUriForFile(mContext, AppUtil.getPackageName(mContext) + FunctionApi
                    .AUTHORITY, apkfile);
