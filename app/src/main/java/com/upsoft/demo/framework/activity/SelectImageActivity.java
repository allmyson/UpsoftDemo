...
public class SelectImageActivity extends BaseActivity {
    ...
    public void initView() {
        ...
        selectPicture.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int mode = selectMode.getCheckedRadioButtonId() == R.id.mode_multiple ? ImageSelectorActivity
                        .MODE_MULTIPLE : ImageSelectorActivity.MODE_SINGLE;
                ...
                ImageSelectorActivity.start(mActivity, maxSelectNum, mode, isShow, isPreview, isCrop);
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK && requestCode == ImageSelectorActivity.REQUEST_IMAGE) {
            ArrayList<String> images = (ArrayList<String>) data.getSerializableExtra(ImageSelectorActivity
                    .REQUEST_OUTPUT);
            String[] paths = images.toArray(new String[images.size()]);
            FunctionApi.LargerImage(mContext, paths, 0);
        }
    }
}
