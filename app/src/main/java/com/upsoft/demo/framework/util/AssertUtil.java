        ...
        try {
            ...
            if (fileList.length > 0) {//如果是目录
                ...
                if (!file.exists() || file.length() == 0) {
                    ...
                    fos.close();
                    L.e("模型文件复制完毕");
//                    ToastUtil.show(activity, "模型文件复制完毕");
                } else {
                    ToastUtil.show(activity, "模型文件已存在，无需复制");
                    L.e("模型文件已存在，无需复制");
                }
            }
        } catch (IOException e) {
    ...
    public static void copyAssetsFile2Phone(Activity activity, String fileName) {
        copyAssetsFile2Phone(activity, activity.getFilesDir().getAbsolutePath(), fileName);
    }

    public static void copyAssetsFile2Phone(Activity activity, String savePath, String fileName) {
        try {
